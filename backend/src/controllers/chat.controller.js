const suzuService = require("../services/anthropic.service");
...
const sendMessage = async (req, res, next) => {
  try {
    ...
    convService.appendMessage(convId, "user", message.trim());
    ...
    if (!reply) {
      throw new Error("No reply returned from suzuService.chat()");
    }

    convService.appendMessage(convId, "assistant", reply);
    if (conv.messages.length <= 2) { const t = await suzuService.generateTitle(message.trim()); convService.rename(convId, t); }
...
const streamMessage = async (req, res, next) => {
  try {
    ...
    res.flushHeaders();
    let fullReply = "";
    await suzuService.chatStream(history, (chunk) => { fullReply += chunk; res.write(`data: ${JSON.stringify({ type: "delta", text: chunk })}\n\n`); }, memories);
    convService.appendMessage(convId, "assistant", fullReply);
    if (conv.messages.length <= 2) { const t = await suzuService.generateTitle(message.trim()); convService.rename(convId, t); }
    res.write(`data: ${JSON.stringify({ type: "done", conversationId: convId })}\n\n`);
    res.end();
  } catch (err) { if (!res.headersSent) return next(err); res.write(`data: ${JSON.stringify({ type: "error", error: err.message })}\n\n`); res.end(); }
};
...
module.exports = { sendMessage, streamMessage, listConversations, newConversation, getConversation, renameConversation, pinConversation, deleteConversation, getMemories, addMemory, deleteMemory };
