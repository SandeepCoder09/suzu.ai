...
api.interceptors.response.use(
  ...
  (err) =>
    Promise.reject(
      new Error(err.response?.data?.error || err.message || "Something went wrong")
    )
);
...
export const streamChatMessage = (
  ...
  fetch(`${BASE}/chat/stream`, {
    ...
    .then(async (res) => {
      if (!res.ok) {
        const e = await res.json().catch(() => ({ error: "Stream failed" }));
        ...
        for (const line of lines) {
          if (line.startsWith("data: ")) {
            try {
              ...
              if (p.type === "delta") onChunk?.(p.text);
              if (p.type === "done") onDone?.(p);
              if (p.type === "error") onError?.(new Error(p.error));
            } catch (_) { }
          }
        }
      }
...
export const listConversations = (q) =>
  api.get("/chat/conversations", { params: q ? { q } : {} });
...
export const renameConversation = (id, title) =>
  api.patch(`/chat/conversations/${id}/rename`, { title });

export const pinConversation = (id) =>
  api.patch(`/chat/conversations/${id}/pin`);

export const deleteConversation = (id) =>
  api.delete(`/chat/conversations/${id}`);

export const clearSession = (conversationId) =>
  api.delete(`/session/${conversationId}`);
